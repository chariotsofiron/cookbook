...
def my_and(arg1: int, arg2: int) -> int:
    ...
    while my_leq(1, my_mul(arg1, arg2)):
        # we look at the first bit of a and b by checking if the number is odd
        # or even. for this we use the modulo operator (a mod 2 and b mod 2)
        # this results in a 1 if the number is odd (which means the first bit
        # 1) and 0 if the number is even (which means the first bit is 0)
        # multiplying those two numbers gives the and-operator for the first
        # bit of a and b. the other bits are handled by bit-shifting a and b
        # same as multiplying (left-shift) or dividing (right-shift) by 2
        result = my_add(
            result, my_mul(my_mul(my_mod(arg1, 2), my_mod(arg2, 2)), temp)
        )
        temp = my_mul(temp, 2)
        arg1 = my_div(arg1, 2)
        arg2 = my_div(arg2, 2)
